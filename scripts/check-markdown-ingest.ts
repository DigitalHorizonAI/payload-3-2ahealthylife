 ...
 * only one of several writers into posts.content. The assertion is on the HTML
 * this site's /api/articles actually serves, which every writer reaches.
 *
 * That path is guarded by Payload itself — the stock convertLexicalToHTMLAsync
 * link converter applies `sanitizeUrl` from payload/shared. We assert it anyway
 * because we make that claim out loud: if a Payload upgrade drops it, we want a
 * red check rather than a silent hole. The React blog page is guarded
 * separately in src/components/Link/index.tsx, which this script cannot reach.
 *
...
// parser (scripts/import-articles.ts) has to handle: a bare <table> with no
// wrapper div, and a top-level <img>. Measured against a real
...
//
// The guard lives at the RENDER sink, not in the ingest hook, because
// posts.content has several writers and only one of them is the hook:
// scripts/import-articles.ts and seed-local.ts write Lexical objects and return
// at the hook's `typeof value !== 'string'` gate, as does the admin panel. So
// this asserts on the HTML /api/articles serves rather than on the stored tree,
// which keeps the original URL by design.
//
// A rejected href renders as '#', not as empty: `sanitizeUrl` from
// payload/shared maps a disallowed protocol to '#'.
//
...
for (const [label, input, expected] of linkCases) {
  ...
  const rendered = renderedHrefs(await convertLexicalToHTMLAsync({ data: tree as never }))
  check(
    'link-safety',
    rendered.length === 1 && rendered[0] === expected,
    `${label}: rendered href ${JSON.stringify(rendered)}, expected ["${expected}"]`,
  )
}
...
and blockquotes intact, and no disallowed link protocol reaches a rendered page.
